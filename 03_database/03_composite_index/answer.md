...
- 使用したクエリ
```sql
SELECT SQL_NO_CACHE * FROM employees.employees WHERE first_name LIKE 'G%' AND gender = 'F';
```
...
- 使用したクエリ
```sql
SELECT SQL_NO_CACHE * FROM employees.employees WHERE last_name IN('Sullins','Vernadat') AND hire_date LIKE '198%';
```
